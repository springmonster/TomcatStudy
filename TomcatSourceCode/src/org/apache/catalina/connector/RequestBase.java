...
/**
 * Convenience base implementation of the <b>Request</b> interface, which can be
 * used for the Request implementation required by most Connectors. Only the
...
public abstract class RequestBase implements ServletRequest, Request {
	...
	/**
	 * Internal notes associated with this request by Catalina components and event
	 * listeners.
	 */
	...
	/**
	 * Set the Context within which this Request is being processed. This must be
	 * called as soon as the appropriate Context is identified, because it
	 * identifies the value to be returned by <code>getContextPath()</code>, and
	 * thus enables parsing of the request URI.
	 *
	...
	/**
	 * Return descriptive information about this Request implementation and the
	...
	/**
	 * Return the Socket (if any) through which this Request was received. This
	 * should <strong>only</strong> be used to access underlying state information
	 * about this Socket, such as the SSLSession associated with an SSLSocket.
	 */
	...
	/**
	 * Set the Wrapper within which this Request is being processed. This must be
	 * called as soon as the appropriate Wrapper is identified, and before the
	 * Request is ultimately passed to an application servlet.
	 *
	...
	/**
	 * Perform whatever actions are required to flush and close the input stream or
	 * reader, in a single operation.
	 *
	 * @exception IOException if an input/output error occurs
	 */
	 ...
	 * this request, or <code>null</code> if no such binding exists.
	 *
	...
	/**
	 * Remove any object bound to the specified name in the internal notes for this
	 * request.
	 *
	...
	/**
	 * Set the content type (and optionally the character encoding) associated with
	 * this Request. For example, <code>text/html; charset=ISO-8859-4</code>.
	 *
	...
	/**
	 * Bind an object to a specified name in the internal notes associated with this
	 * request, replacing any existing binding for this name.
	 *
	 ...
	 * <code>Enumeration</code> if there are none.
	 */
	...
	/**
	 * Return the servlet input stream for this Request. The default implementation
	 * returns a servlet input stream created by <code>createInputStream()</code>.
	 *
	 * @exception IllegalStateException if <code>getReader()</code> has already been
	...
	/**
	 * Return the preferred Locale that the client will accept content in, based on
	 * the value for the first <code>Accept-Language</code> header that was
	...
	/**
	 * Return the set of preferred Locales that the client will accept content in,
	 * based on the values for any <code>Accept-Language</code> headers that were
	 * encountered. If the request did not specify a preferred language, the
	 * server's default Locale is returned.
	 */
	 ...
	 *
	 * @return A <code>Map</code> containing parameter names as keys and parameter
	 ...
	 * implementation wraps a <code>BufferedReader</code> around the servlet input
	 * stream returned by <code>createInputStream()</code>.
	 *
	 * @exception IllegalStateException if <code>getInputStream()</code> has already
