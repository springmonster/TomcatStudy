...
/**
 * Implementation of a request processor (and its associated thread) that may be
 * used by an HttpConnector to process individual requests. The connector will
 * allocate a processor from its pool, assign a particular socket to it, and the
 * processor will then execute the processing required to complete the request.
 * When the processor is completed, it will recycle itself.
 *
...
final class HttpProcessor implements Lifecycle, Runnable {
    ...
    /**
     * The socket we are currently processing a request for. This object is used for
    ...
    /**
     * True if the client has asked to recieve a request acknoledgement. If so the
     * server will send a preliminary 100 Continue response just after it has
     * successfully parsed the request headers, and before starting reading the
     * request entity body.
     */
    ...
    /**
     * Process an incoming TCP/IP connection on the specified socket. Any exception
     * that occurs during processing must be logged and swallowed. <b>NOTE</b>: This
     * method is called from our Connector's thread. We must assign it to our own
     * thread so that multiple simultaneous requests can be handled.
     *
    ...
    /**
     * Await a newly assigned Socket from our Connector, or <code>null</code> if we
     ...
     * corresponding Locales to the current request.
     *
    ...
    /**
     * Parse the incoming HTTP request headers, and set the appropriate request
     * headers.
     *
    ...
    /**
     * Process an incoming HTTP request on the Socket that has been assigned to this
     * Processor. Any exceptions that occur during processing must be swallowed and
     * dealt with.
     *
